  ...
  input_prefix(input_prefix_)
{
  ...
  {
    VERBOSE("failed!");
    SSRERROR("File must be writable and needs an extension recognized by "
        "ecasound, e.g. \".wav\".");
...
#ifdef _WIN32
  // this_thread::sleep std::duration chrono::
  Sleep(ssr::usleeptime / 1000);
#else
...
bool AudioRecorder::enable()
{
  ...
  if (_eca.error())
  {
    SSRERROR("ecasound: " + _eca.last_error());
    return false;
  }
  return true;
}
